    ...
    <DefaultLayout seo={{ ...placeholderSEO, ...seo }}>
      <PageHero className="container">
        ...
        <section className={heroStyles.ctaSection}>
          <Link href={'/registry'} className={`btn btn-default ${heroStyles.ctaBtn}`}>
      ...
      <section className={'container'}>
        <IDLNav idl={record?.idl} options={SUBNAV_OPTIONS} selected={selectedTab} />
      </section>
      ...
      {(record?.idl as Idl) ? (
        <>
          ...
          {selectedTab == 'accounts' && <IDLAccountsTable data={record.idl?.accounts} />}
          {selectedTab == 'types' && <IDLTypesTable data={record.idl?.types} />}
          {selectedTab == 'errors' && <IDLErrorsTable data={record.idl?.errors} />}
          {selectedTab == 'constants' && <IDLConstantsTable data={record.idl?.constants} />}
          {selectedTab == 'events' && <IDLEventsTable data={record.idl?.events} />}
        </>
...
function RenderTable({ tab, record }: { tab: string; record: IDLRecord }) {
  switch (tab) {
    ...
    default:
      return (
        <p className="p-8 text-center text-2xl">Unknown page. Please select a valid option.</p>
      );
  }
}
