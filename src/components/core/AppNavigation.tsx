...
export default function AppNavigation({}: ComponentProps) {
  const [openState, setOpenState] = useState(new Array(NAVIGATION_ITEMS.length).fill(false));

  // function to update the openState for each of the menu options
  function handleUpdateState(position: number, value: boolean) {
    setOpenState(openState.map((item, index) => (index === position ? value : false)));
  }
        ...
        >
          ...
          >
            ...
            <span className={styles.arrow}>
              <ChevronDownIcon className={`icon-md ${openState[id] && 'rotate-180'} `} />
            </span>
          </div>
          {/* <Transition
              show={openState[id]}
              ...
              <div className={styles.dropdownInner}>
                <div className={styles.primaryList}>
                  {section.links.map((item, id) => (
                    ...
                    >
                      {item?.icon !== undefined && (
                        <span className={`${styles.iconArea} ${item?.className || ''}`}>
                        ...
                        <p className={styles.label}>
                          ...
                          {item?.isExternal && (
                            <span>
                              ...
                              <ArrowUpRightIcon className="icon ml-2" />
                            </span>
                          )}
                        </p>
                        {item.description && (
                ...
                {section?.secondaryLinks && (
                  <div className={styles.secondaryList}>
                    {section.secondaryLinks?.href ? (
                      <Link href={section.secondaryLinks.href} className={styles.heading}>
                        {section.secondaryLinks.label}
                      </Link>
                    ) : (
                      <p className={styles.heading}>{section.secondaryLinks.label}</p>
                    )}
                      ...
                      >
                        ...
                        {item?.isExternal && (
                          <span>
                            ...
                            <ArrowUpRightIcon className="icon-sm ml-2" />
                          </span>
                        )}
                      </Link>
                    ))}
